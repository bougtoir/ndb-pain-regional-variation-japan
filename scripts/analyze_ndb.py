...
CLASS_EN = {
    114: 'antipyretic analgesics including NSAIDs and acetaminophen',
    811: 'opium alkaloid narcotics including morphine and codeine',
    821: 'synthetic narcotics including fentanyl and pethidine',
}
...
for cls, name in ANALGESIC_CLASSES.items():
    ...
    tohoku_mean = float(tohoku_arr.mean())
    tohoku_sd = float(tohoku_arr.std())
    rest_mean = float(rest_arr.mean())
    rest_sd = float(rest_arr.std())
    ...
    class_records.append({
        'class_code': cls,
        'class_name_en': CLASS_EN[cls],
        'tohoku_n': len(tohoku_arr),
        'tohoku_mean': tohoku_mean,
        'tohoku_sd': tohoku_sd,
        'non_tohoku_n': len(rest_arr),
        'non_tohoku_mean': rest_mean,
        'non_tohoku_sd': rest_sd,
        'mann_whitney_u': float(u),
        'p_value': float(p),
    })
    print(f"  Tohoku mean: {tohoku_mean:.4f} (SD: {tohoku_sd:.4f})")
    print(f"  Rest mean: {rest_mean:.4f} (SD: {rest_sd:.4f})")
    if len(tohoku_arr) > 0 and len(rest_arr) > 0:
        print(f"  Mann-Whitney U: U={u:.1f}, p={p:.4f}")

class_summary_df = pd.DataFrame(class_records)
class_summary_df.to_csv(OUTPUT_DIR + 'analgesic_class_summary.csv', index=False, encoding='utf-8-sig')
print(f"\nSaved: {OUTPUT_DIR}analgesic_class_summary.csv")
